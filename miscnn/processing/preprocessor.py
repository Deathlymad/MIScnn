...
from miscnn.processing.subfunctions.sample_splitting import SampleSplitting
...
class Preprocessor:
    ...
    def __init__(self, data_io, batch_size, subfunctions=[],
                 ...
                 prepare_batches=False, sample_splitter=SampleSplitting(), use_multiprocessing=False):
    ...
    def run(self, indices_list, training=True, validation=False):
        ...
        for index in indices_list:
            ...
            if (data_aug):
                self.sample_splitter.set_data_aug(self.data_augmentation)
            
            self.sample_splitter.preprocessing(sample, training)
            
            if training : ready_data = list(zip(sample.img_data, sample.seg_data))
            else : ready_data = list(zip(sample.img_data))
    ...
    def postprocessing(self, sample, prediction, activation_output=False):
        ...
        self.sample_splitter.postprocessing(sample, prediction)
